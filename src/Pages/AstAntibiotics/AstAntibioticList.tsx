          ...
          <div className="relative transform overflow-hidden rounded-xl bg-white text-left shadow-2xl ring-1 ring-gray-100 transition-all sm:my-8 sm:w-full sm:max-w-md" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="bg-white px-6 pb-6 pt-7 sm:p-8">
              <div className="flex items-start gap-4 mb-6">
                <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-blue-100 text-blue-600 ring-4 ring-blue-50">
                  <i className="fa-solid fa-capsules text-lg"></i>
                </div>
                <div>
                  <h3 className="text-xl font-bold leading-7 text-gray-900">
                    {item ? 'Edit AST Antibiotic' : 'New AST Antibiotic'}
                  </h3>
                  <p className="mt-1 text-sm text-gray-500">
                    {item ? 'Edit the antibiotic name' : 'Enter the name of the new antibiotic'}
                  </p>
                </div>
              </div>
              <Input label="Name *" type="text" name="antibioticName" value={name} onChange={(e) => { setName(e.target.value); if (error) setError('') }} error={error ? { message: error } : undefined} />
              <div className="mt-6 flex justify-end gap-3">
                ...
                <button type="button" className={`inline-flex min-w-28 justify-center rounded-md px-4 py-2 text-sm font-semibold text-white shadow-sm cursor-pointer ${saving ? 'bg-blue-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-500'}`} onClick={handleSave} disabled={saving}>
...
const AstAntibioticList = () => {
  ...
  const handleSave = async (name: string) => {
    ...
    try {
      if (formModal.item) {
        await updateItem(ENTITIES.AST_ANTIBIOTICS, formModal.item.id, { name })
        toast.success('Antibiotic updated successfully')
      } else {
        await createItem(ENTITIES.AST_ANTIBIOTICS, { name })
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteItem(ENTITIES.AST_ANTIBIOTICS, deleteModal.item.id)
  ...
  return (
    <div>
      <PageHeader icon="fa-solid fa-capsules" title="AST Antibiotics" subtitle={`${totalCount} antibiotics total`} buttons={[{ label: 'New Antibiotic', icon: 'fa-solid fa-plus', onClick: () => setFormModal({ isOpen: true, item: null }), variant: 'primary' }]} />
      ...
      <DeleteModal isOpen={deleteModal.isOpen} onClose={() => setDeleteModal({ isOpen: false, item: null, isLoading: false })} onConfirm={handleDelete} title="Delete AST antibiotic" description={`Are you sure you want to delete "${deleteModal.item?.name || ''}"? This action is irreversible.`} isLoading={deleteModal.isLoading} />
    </div>
  )
}

export default AstAntibioticList
