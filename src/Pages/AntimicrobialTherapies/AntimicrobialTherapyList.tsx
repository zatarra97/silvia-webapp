              ...
              <div className="flex items-start gap-4 mb-6">
                ...
                <div>
                  <h3 className="text-xl font-bold leading-7 text-gray-900">
                    {item ? 'Edit Antimicrobial Therapy' : 'New Antimicrobial Therapy'}
                  </h3>
                  <p className="mt-1 text-sm text-gray-500">
                    {item ? 'Edit the therapy name' : 'Enter the name of the new therapy'}
                  </p>
                </div>
              </div>
              <Input
...
const AntimicrobialTherapyList = () => {
  ...
  const handleSave = async (name: string) => {
    ...
    try {
      if (formModal.item) {
        await updateItem(ENTITIES.ANTIMICROBIAL_THERAPIES, formModal.item.id, { name })
        toast.success('Therapy updated successfully')
      } else {
        await createItem(ENTITIES.ANTIMICROBIAL_THERAPIES, { name })
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteItem(ENTITIES.ANTIMICROBIAL_THERAPIES, deleteModal.item.id)
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={`${totalCount} therapies total`}
        buttons={[
          {
            label: 'New Therapy',
      ...
      <div className="container mx-auto p-6">
        <Table columns={columns} data={therapies} actions={actions} loading={loading} />
        {totalPages > 1 && (
          <Pagination
      ...
      <DeleteModal
        ...
        title="Delete antimicrobial therapy"
        description={`Are you sure you want to delete "${deleteModal.item?.name || ''}"? This action is irreversible.`}
...
export default AntimicrobialTherapyList
